...
const ContributeModal: React.FC<Props> = ({ currency, contract, currencyAddress, onDismiss,decimals }) => {
  const [value, setValue] = useState('')
  const [pendingTx, setPendingTx] = useState(false)
  const { account } = useWallet()
  const balance = getFullDisplayBalance(useTokenBalance(currencyAddress), decimals)
